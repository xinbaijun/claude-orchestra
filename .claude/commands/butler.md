...
- **派活到现成 worker**：任何时刻 push（worker 在工作 / thinking / 输入框有 auto-suggest / 用户 attached 都不顾忌）—— cc queue 机制可信，FIFO 处理不丢。详见 `.claude/commands/dispatch.md` Step 6.7。**唯一护栏**：管家自己记账每个 worker push 了几个 task，事后核对完工数 vs 派发数。
...
- 「这个 worker 长 idle 不想推了 / 想挂起 / kill 算了」 / 用户主动说"等触发条件" / "想做但没动力" → 走**两步法**（详见 CLAUDE.md「长 idle worker 处理（两步法）」段）：
  ...
  3. 派 chore-runner 把该 worker 的 board 行从「活跃对话」搬到「历史活跃」（或下次 /peek 一致性 diff 自动检测）
...
- 管家 Edit **默认不 commit** —— butler_decisions.md 已加进 race trailer 排除清单
- **batch commit 由 chore-runner 触发**：管家跑 `/peek` Step 6 检测 butler_decisions dirty + 累积 ≥ 3 条新决策（或距上次 commit > 2h）→ 派 chore-runner 顺手 commit；或用户口语"小弟 commit 一下管家决策"显式触发
- **应急 commit**：管家自己跑 `${GIT_WRAPPER} add` + `${GIT_WRAPPER} commit` 单文件落地仍允许（仅紧急 case 用）；默认 batch 不走这条路
